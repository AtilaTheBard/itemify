...
import { useTweaks, TweaksPanel, TweakSection, TweakSlider, TweakColor } from './TweaksPanel'
import './App.css'

/* ── constants ───────────────────────────────────────── */
const TWEAK_DEFAULTS = { accent:'#ffffff', voteWindowSeconds:30 }
const ACCENT_OPTIONS = ['#ffffff','#ff7849','#4ade80','#7aa2ff','#c4b5fd','#fcd34d']
...
/* ── VideoCard (chat message type) ──────────────────── */
function VideoCard({msg,myId}) {
  ...
  return (
    <div className={`chat-msg ${isMe?'mine':''}`}>
      ...
      <div className="chat-bubble">
        ...
        <div className="vc">
          <img src={msg.thumb} alt="" className="vc-thumb"/>
          <div className="vc-body">
            <div className="vc-title">{msg.title}</div>
            {msg.channel&&<div className="vc-ch">{msg.channel}</div>}
            <span className="vc-badge"><Icon.Film/>sıraya eklendi</span>
          </div>
        </div>
      </div>
    </div>
...
/* ── ChatSidebar ─────────────────────────────────────── */
function ChatSidebar({messages,onSend,myId,onClose}) {
  ...
  const isLink = !!extractVideoId(text.trim())
  ...
  return (
    <aside className="chat-sidebar">
      <div className="chat-sidebar-head">
        <h3>Sohbet</h3>
        <button className="chat-close-btn" onClick={onClose} title="Sohbeti gizle"><Icon.X/></button>
      </div>
      <div className="chat-messages">
        {!messages.length&&<div className="empty">henüz mesaj yok — merhaba de!</div>}
        {messages.map((msg,i)=>(
          msg.type==='video'
            ? <VideoCard key={msg.timestamp+'-'+i} msg={msg} myId={myId}/>
      ...
      <form className="chat-input-row" onSubmit={submit}>
        <input
          ref={inputRef}
          className={`chat-field ${isLink?'link-mode':''}`}
          placeholder={isLink?'YouTube linki algılandı — gönder':'mesaj yaz veya YouTube linki yapıştır…'}
          value={text}
          onChange={e=>setText(e.target.value)}
          maxLength={300}
        />
        <button type="submit" className={`chat-send ${isLink?'link-send':''}`} disabled={!text.trim()}>
          {isLink?<Icon.Plus/>:<Icon.Send/>}
        </button>
      </form>
    </aside>
  )
        ...
        <div className="share-body">
          <div className="share-link-wrap">
            ...
            <button className="share-copy-btn" onClick={copy}>
              <Icon.Copy/>{copied?'Kopyalandı!':'Kopyala'}
            </button>
          </div>
          {roomCode&&<p className="share-hint">Arkadaşların oda kodu <strong>{roomCode}</strong> ile de katılabilir.</p>}
        </div>
      </div>
    </div>
  )
...
function App({room,onLeave}) {
  ...
  useEffect(()=>{
    ...
    const unsubH=onValue(query(ref(db,`rooms/${room}/history`),limitToLast(30)),snap=>{const d=snap.val()||{};setHistory(Object.values(d).sort((a,b)=>(b.playedAt||0)-(a.playedAt||0)))})
  ...
  const sendMessage=useCallback(async text=>{
    ...
    if(videoId){
      let title='Video yükleniyor…',channel=''
      try{
        const r=await fetch(`https://www.youtube.com/oembed?url=https://youtu.be/${videoId}&format=json`)
        const d=await r.json()
        title=d.title||title; channel=d.author_name||''
      }catch{}
      const msg={userId:MY_ID,name:myName,type:'video',videoId,title,channel,thumb:`https://img.youtube.com/vi/${videoId}/mqdefault.jpg`,timestamp:Date.now()}
      await push(ref(db,`rooms/${room}/chat`),msg)
      const track={id:uid(),videoId,title,duration:0,requester:{name:myName,id:MY_ID},thumb:`https://img.youtube.com/vi/${videoId}/mqdefault.jpg`,source:'user',createdAt:Date.now()}
  ...
  const closeRoom=useCallback(async()=>{
    if(!confirm('Odayı kapatmak istediğinden emin misin?'))return
    await set(ref(db,`rooms/${room}/meta/closed`),true)
  },[room])

  const openShare=useCallback(()=>setShareLink(window.location.href),[])
  ...
  return (
    <>
      <div className="backdrop"/><div className="bloom b1"/><div className="bloom b2"/><div className="bloom b3"/>
      {historyOpen&&<HistoryModal items={history} onClose={()=>setHistoryOpen(false)}/>}
        ...
        <header className="topbar">
          ...
          <div className="topbar-meta">
            ...
            <div className="topbar-actions">
              <button className="topbar-action" onClick={()=>setHistoryOpen(true)} title="Geçmiş"><Icon.History/></button>
              <button className="topbar-action" onClick={openShare} title="Oda linkini paylaş"><Icon.Share/></button>
              <button className="topbar-action danger" onClick={closeRoom} title="Odayı kapat"><Icon.Power/></button>
              <button className={`topbar-action ${chatOpen?'active':''}`} onClick={()=>setChatOpen(o=>!o)} title={chatOpen?'Sohbeti gizle':'Sohbeti göster'}><Icon.Chat/></button>
            </div>
          </div>
        </header>

        <div className={`grid ${chatOpen?'with-chat':''}`}>
          <div className="main-col">
            ...
            {!nowPlaying&&(
              <div className="empty-stage">
                ...
                <span>sohbet kutusuna bir YouTube linki yapıştır</span>
              </div>
            )}
          </div>
